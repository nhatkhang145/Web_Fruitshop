...
public class WeekendDealDAO {
    public List<WeekendDeal> getActiveDeals() {
        String sql = "SELECT wd.id, wd.product_id, wd.tag, wd.subtitle, wd.discount_percent, " +
                ...
                "WHERE wd.status = 1 " +
                "  AND NOW() BETWEEN wd.start_date AND wd.end_date " +
    ...
    public WeekendDeal getActiveDealByProductId(int productId) {
        String sql = "SELECT wd.id, wd.product_id, wd.tag, wd.subtitle, wd.discount_percent, " +
                "       wd.start_date, wd.end_date, wd.status, wd.sort_order, wd.created_at, " +
                "       p.id as p_id, p.name, p.product_code, p.price, p.sale_price, " +
                "       p.quantity, p.short_description, p.image, p.category_id, p.status as p_status " +
    ...
    public java.util.Map<Integer, WeekendDeal> getActiveDealsByProductIds() {
        String sql = "SELECT wd.id, wd.product_id, wd.tag, wd.subtitle, wd.discount_percent, " +
                "       wd.start_date, wd.end_date, wd.status, wd.sort_order, wd.created_at, " +
                "       p.id as p_id, p.name, p.product_code, p.price, p.sale_price, " +
                "       p.quantity, p.short_description, p.image, p.category_id, p.status as p_status " +
        ...
        List<WeekendDeal> deals = DBContext.get().withHandle(handle ->
                handle.createQuery(sql)
                        .map(new WeekendDealWithProductMapper())
                        .list()
        );

        java.util.Map<Integer, WeekendDeal> dealMap = new java.util.HashMap<>();
        for (WeekendDeal deal : deals) {
            dealMap.put(deal.getProductId(), deal);
        }
        return dealMap;
    }
