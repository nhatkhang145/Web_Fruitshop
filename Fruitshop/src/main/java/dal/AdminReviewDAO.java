...
public class AdminReviewDAO {
    ...
    public double getAverageRatingByProductId(int productId) {
        String query = "SELECT AVG(CAST(rating AS DECIMAL(10,2))) FROM reviews WHERE product_id = :pid AND status = 'approved'";
        return DBContext.get().withHandle(handle ->
                handle.createQuery(query)
                        .bind("pid", productId)
                        .mapTo(Double.class)
    ...
    public int getReviewCountByProductId(int productId) {
        String query = "SELECT COUNT(*) FROM reviews WHERE product_id = :pid AND status = 'approved'";
        return DBContext.get().withHandle(handle ->
                handle.createQuery(query)
                        .bind("pid", productId)
                        .mapTo(Integer.class)
