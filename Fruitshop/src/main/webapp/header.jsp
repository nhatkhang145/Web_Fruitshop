                        ...
                        <div class="navbar__content">
                            <ul class="navbar-list">
                                <li class="navbar-item navbar-item--no-click">
                                    Kết nối:
                                    <a href="#" class="navbar-icon__link"><i
                                            class="navbar-icon fa-brands fa-facebook"></i></a>
                                    <a href="#" class="navbar-icon__link"><i
                                            class="navbar-icon fa-brands fa-instagram"></i></a>
                                </li>
                            </ul>
                            ...
                            <ul class="navbar-list">
                                <li class="navbar-item">
                                    <a href="${pageContext.request.contextPath}/faqs.jsp" class="navbar-item__link">
                                        <span class="navbar-icon__link"><i
                                                class="navbar-icon fa-regular fa-circle-question"></i></span> Hỗ trợ
                                    </a>
                                </li>
                            ...
                            <div class="navbar__menu-search">
                                <form class="search-form" action="${pageContext.request.contextPath}/shop" method="get">
                                    <input name="q" class="search-input" type="search"
                                        placeholder="Tìm kiếm sản phẩm..." value="${param.q}"
                                        style="width: 100%; border-radius: 25px 0 0 25px;" />
                                            ...
                                            <div class="account-auth">
                                                ...
                                                <div class="auth-buttons">
                                                    <a href="login.jsp" class="btn--primary"
                                                        style="display: block; text-align: center; margin-bottom: 8px;">Đăng
                                                        nhập</a>
                                                    <a href="register.jsp" class="btn--outline"
                                                        style="display: block; text-align: center;">Đăng ký</a>
                                                </div>
                                            </div>
                                        </c:if>
                                        <c:if test="${not empty sessionScope.account}">
                                            <div class="account-user">
                                                ...
                                                <ul class="account-menu">
                                                    <li><a href="profile"><i class="fa-solid fa-id-card"></i> Hồ sơ cá
                                                            nhân</a></li>
                                                    <li><a href="orders"><i class="fa-solid fa-box-open"></i> Đơn
                                                            hàng</a></li>

                                                    <%-- Chỉ hiển thị đổi mật khẩu nếu là tài khoản local --%>
                                                        <c:if
                                                            test="${empty sessionScope.account.loginType or sessionScope.account.loginType == 'local'}">
                                                            <li><a href="change-password.jsp"><i
                                                                        class="fa-solid fa-key"></i> Đổi mật khẩu</a>
                                                            </li>
                                                        </c:if>

                                                        <c:if
                                                            test="${sessionScope.account.role == 1 or (sessionScope.account.roleId != null and sessionScope.account.roleId > 0)}">
                                                            <li>
                                                                <a href="${pageContext.request.contextPath}${not empty sessionScope.adminLandingPath ? sessionScope.adminLandingPath : '/admin/dashboard'}"
                                                                    style="color: #007bff;">
                                                                    <i class="fa-solid fa-user-shield"></i>
                                                                    <c:choose>
                                                                        <c:when
                                                                            test="${sessionScope.account.role == 1}">
                                                                            Trang tổng quan</c:when>
                                                                        <c:when
                                                                            test="${fn:contains(fn:toLowerCase(sessionScope.adminLandingPath), 'inventory-warehouse')}">
                                                                            Quản lý kho</c:when>
                                                                        <c:when
                                                                            test="${fn:contains(fn:toLowerCase(sessionScope.adminLandingPath), 'orders')}">
                                                                            Quản lý đơn hàng</c:when>
                                                                        <c:when
                                                                            test="${fn:contains(fn:toLowerCase(sessionScope.adminLandingPath), 'users')}">
                                                                            Quản lý khách hàng</c:when>
                                                                        <c:when
                                                                            test="${fn:contains(fn:toLowerCase(sessionScope.adminLandingPath), 'roles')}">
                                                                            Phân quyền</c:when>
